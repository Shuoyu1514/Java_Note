...
![img](./5.Spring%E9%9D%A2%E8%AF%95%E9%A2%98.assets/1716791217083-132fe2ba-706a-4d8d-aef3-1617a2046585.png)
...
> 但可以通过注入自身代理或使用AopContext来间接实现这一需求
...
![image-20250401092903472](./5.Spring%E9%9D%A2%E8%AF%95%E9%A2%98.assets/image-20250401092903472.png)
...
- 实例化 Bean：当 Spring 实例化一个 Bean 后，会在其生命周期中调用 postProcessProperties() 方法
...
1. 拦截器注册：PageHelper 借助实现 MyBatis 的 Interceptor 接口，创建一个拦截器类。在配置 MyBatis 时，会把这个拦截器注册到 MyBatis 的==拦截器==链中。当执行 SQL 查询时，MyBatis 会按照拦截器链的顺序依次调用各个拦截器。
2. 分页参数设置：PageHelper.startPage(pageNum, pageSize) 方法来设置分页参数，这些参数会保存在==threadlocal==中
...
当实现方提供了接口和实现，我们可以通过调用实现方的接口从而拥有实现方给我们提供的能力，这就是 API。这种情况下，接口和实现都是放在实现方的包中。

调用方通过接口调用实现方的功能，而不需要关心具体的实现细节。当接口存在于调用方这边时，这就是 SPI 。由接口调用方确定接口规则，然后由不同的厂商根据这个规则对这个接口进行实现，从而提供服务。

![image-20250401214951629](./5.Spring%E9%9D%A2%E8%AF%95%E9%A2%98.assets/image-20250401214951629.png)

### * 注入Bean的方式

| **方式**        | **描述**                            | **推荐使用场景**                     |
| --------------- | ----------------------------------- | ------------------------------------ |
| XML             | 最早方式，灵活但繁琐                | 老项目、复杂 XML 配置                |
| @Component 注解 | 自动扫描，配合 Spring Boot 非常方便 | 普通 Bean、服务、控制器等            |
| @Bean 注解      | 编程式注册 Bean，可自定义构造参数   | 需要手动控制依赖、第三方类无法加注解 |
| 动态注册        | 运行时动态添加 Bean                 | 写框架/插件/SDK 等高级场景           |

> 自己写的类 → @Component（让 Spring 自动扫描，类上注解）
> 别人写的类 → @Bean（你手动构造、注册，方法上注解）

### Springboot热部署

DevTools，JRebel

DevTools 实际是通过 “==双 ClassLoader==” 实现的 自动重启！

重启的是 Spring 应用上下文，不是 JVM，不是真正全局重启

```
原始类加载器：ClassLoaderA
修改了 UserController.java → 重新编译 → DevTools 创建 ClassLoaderB 加载新类
旧对象销毁，Spring 重新初始化新 Bean 实例
```
