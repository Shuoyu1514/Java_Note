...
Sync 通常指的是：某个并发组件内部使用 AQS 的 “同步器” 实现类，也就是一个封装了 AQS（AbstractQueuedSynchronizer） 的子类
...
![image-20250405164018056](./3.Java%E5%B9%B6%E5%8F%91%E7%BC%96%E7%A8%8B%E9%9D%A2%E8%AF%95%E9%A2%98.assets/image-20250405164018056.png)
...
当我们在同一个线程中声明了两个 `ThreadLocal` 对象的话， `Thread`内部都是**使用仅有的那个`ThreadLocalMap` 存放数据**的，`ThreadLocalMap`的 key 就是 `ThreadLocal`对象，value 就是 `ThreadLocal` 对象调用`set`方法设置的值。

```
Thread A
 └── threadLocals（ThreadLocalMap）
       ├── key: ThreadLocal1 → value: User1
       ├── key: ThreadLocal2 → value: Token123
       └── ...

Thread B
 └── threadLocals（ThreadLocalMap）
       ├── key: ThreadLocal1 → value: User2
       └── ...
```
...
ThreadLocal的**key是弱引用**，如果 `ThreadLocal` 实例不再被任何强引用指向，垃圾回收器会在下次 GC 时回收该实例，导致 `ThreadLocalMap` 中对应的 key 变为 `null`

 **value 是强引用**, 即使 `key` 被 GC 回收，`value` 仍然被 `ThreadLocalMap.Entry` 强引用存在，无法被 GC 回收。

线程池中的线程是复用的！如果 value 是大对象（如请求上下文、数据库连接等），就会堆积在内存。最终：==导致内存泄漏（堆外对象无法回收）==

### 如何跨线程传递 ThreadLocal 的值？

如果想要在异步场景下传递 ThreadLocal 值，有两种解决方案：

- ==InheritableThreadLocal==： InheritableThreadLocal 是JDK1.2 提供的工具，继承自 ThreadLocal 。使用 InheritableThreadLocal 时，会在创建子线程时，令子线程继承父线程中的 ThreadLocal 值， 但是无法支持线程池场景下的 ThreadLocal 值传递。
...
线程池内部使用==一个变量维护两个值==：运行状态(runState)和线程数量 (workerCount)

#### worker

线程池为了掌握线程的状态并维护线程的生命周期，设计了线程池内的工作线程Worker。

> Worker 是 ThreadPoolExecutor 的一个内部类，用来封装线程执行逻辑和生命周期控制。
...
+   **keepAliveTime**：==线程存活时间==。当线程池中线程的数量大于corePoolSize，并且某个线程的空闲时间超过了keepAliveTime，那么这个线程就会被销毁。
    +   

+   **unit**：就是keepAliveTime时间的==单位==。
+   **workQueue**：==工作队列==。当没有空闲的线程执行新任务时，该任务就会被放入工作队列中，等待执行。
+   **threadFactory**：==线程工厂==。可以用来<u>给线程取名字，创建新线程</u>等等
+   **handler**：==拒绝策略==。当一个新任务交给线程池，如果此时线程池中有空闲的线程，就会直接执行，如果没有空闲的线程，就会将该任务加入到阻塞队列中，如果阻塞队列满了，就会创建一个新线程，从阻塞队列头部取出一个任务来执行，并将新任务加入到阻塞队列末尾。如果当前线程池中线程的数量等于maximumPoolSize，就不会创建新线程，就会去执行拒绝策略

### * 临时线程销毁过程？销毁的一定是后来创建的临时线程吗？

1. 当任务执行完毕，线程变为空闲状态。
2. 如果该线程是临时线程，并且空闲时间超过了 keepAliveTime：
3. 线程池中的 Worker 线程会自我判断是否需要退出。
4. 如果是，线程就会从线程池的 workers 集合中移除，并终止。

❌ 不一定。销毁的是哪一个线程，跟创建顺序没有必然关系，而是跟空闲时间的长短有关。

### **线程池阻塞队列的类型**

- `ArrayBlockingQueue`（有界阻塞队列）：基于==数组==的有界队列，初始化时需指定固定容量（capacity），一旦创建，容量不可变。**先进先出**
- `LinkedBlockingQueue`（同步队列）：基于==单链表==的可选有界队列，默认无界（Integer.MAX_VALUE），也可指定容量**先进先出**
- `SynchronousQueue`（同步队列）：没有容量，不存储元素，目的是保证对于提交的任务，如果有空闲线程，则使用空闲线程来处理；否则新建一个线程来处理任务
- `DelayedWorkQueue`（延迟队列）：按照延迟的时间长短对任务进行排序，内部采用的是“堆”的数据结构，可以保证每次出队的任务都是当前队列中执行时间最靠前的
- `PriorityBlockingQueue`（优先级队列）：任务按优先级排序（需实现 Comparable 或提供 Comparator）

> 内置线程池本质就是 ThreadPoolExecutor
>
> 只有ArrayBlockingQueue是有界的，LinkedBlockingQueue默认无界（可设置有界）
...
+   ==CallerRunsPolicy==，<u>使用线程池的调用者所在的线程去执行被拒绝的任务</u>，除非线程池被停止或者线程池的任务队列已有空缺。
    +   可能出现的问题：耗时的任务用了主线程执行，导致线程池阻塞，进而导致后续任务无法及时执行
    +   解决：任务持久化

+   ==AbortPolicy==，直接<u>抛出一个任务被线程池拒绝的异常</u>。
+   ==DiscardPolicy==，不做任何处理，<u>静默拒绝</u>提交的任务。
+   ==DiscardOldestPolicy==，**<u>抛弃最老的任务**，然后执行该任务</u>。
+   ==自定义拒绝策略==，通过实现接口可以自定义任务拒绝策略。

###  *** 有线程池参数设置的经验吗？**

> 实际开发中通常用==动态线程池==参数进行配置
...
- 使用 **execute（）**提交任务：当任务通过execute（）提交到线程池并在执行过程中抛出异常时，如果这个异常没有在任务内被捕获，那么==该异常会导致当前线程终止==，并且异常会被打印到控制台或日志文件中。==线程池会检测到这种线程终止，并创建一个新线程来替换它，从而保持配置的线程数不变==。
...
《阿里巴巴 Java 开发手册》中提到，禁止使用这些方法来创建线程池，而应该==手动 new ThreadPoolExecutor 来创建线程池==。

- 使用有界队列，控制线程创建数量，避免OOM
- 实际使用中需要根据自己机器的性能、业务场景来手动配置线程池的参数比如核心线程数、使用的任务队列、饱和策略等等。
- 我们应该显示地给我们的线程池命名，这样有助于我们定位问题。

> 监测线程池运行状态：SpringBoot 中的 Actuator 组件
>
> 建议不同类别的业务用不同的线程池
>
> 别忘记给线程池命名
...
先收集线程转储(jstack)、堆转储(jmap)和GC日志，再针对性地分析解决。

线程耗尽：调整线程池配置

死锁：分析线程转储找出死锁位置；修改同步代码逻辑

资源耗尽：增加JVM内存(-Xmx)；检查内存泄漏

### * 线程池如何收集异常？

使用==自定义线程池 + afterExecute() + 日志上报系统（如 ELK、Sentry 等）==来捕获和处理线程异常
...
![image-20250405202658532](./3.Java%E5%B9%B6%E5%8F%91%E7%BC%96%E7%A8%8B%E9%9D%A2%E8%AF%95%E9%A2%98.assets/image-20250405202658532-3856019.png)

但上面并没有动态指定==队列长度==的方法，美团的方式是自定义了一个叫做 `ResizableCapacityLinkedBlockIngQueue` 的队列，主要就是把 LinkedBlockingQueue 的 capacity 字段的 final 关键字修饰给去掉了，让它变为可变的。

### Spring项目中如何使用线程池？

在配置类里中创建，然后在需要异步执行的方法上使用，不管你有多少个请求过来，这些方法调用都会复用这个名为 myExecutor 的线程池实例，线程池由 Spring 管理，只初始化一次

> 如果你在方法内部或控制器里每次都 **new** 一个线程池，那就真的会创建很多线程池

### Future

Future 类只是一个泛型接口，其中定义了 5 个方法。
...
![image-20250413142033244](./3.Java%E5%B9%B6%E5%8F%91%E7%BC%96%E7%A8%8B%E9%9D%A2%E8%AF%95%E9%A2%98.assets/image-20250413142033244.png)

### 对比future和CompletableFuture

Future：阻塞，

CompletableFuture：支持非阻塞回调；支持链式调用、异步流式处理；函数式编程

### 一个任务需要依赖另外两个任务执行完之后再执行，怎么设计？

任务编排场景非常适合通过CompletableFuture实现
...
当同时提交大量任务时，可能会导致资源竞争和线程饥饿，进而影响系统性能。

### execute() vs submit()

- 返回值：
  - execute（）方法用于提交不需要返回值的任务。通常用于执行 Runnable 任务，无法判断任务是否被线程池成功执行。
  - submit（）方法用于提交需要返回值的任务。可以提交 Runnable 或 Callable 任务。submit（）方法返回一个 Future 对象，通过这个 Future 对象可以判断任务是否执行成功，并获取任务的返回值
- 异常处理：在使用 submit（）方法时，可以通过 Future 对象处理任务执行过程中抛出的异常；而在使用 execute（）方法时，异常处理需要通过自定义的 ThreadFactory（在线程工厂创建线程的时候设置 UncaughtExceptionHandler 对象来处理异常）或 ThreadPoolExecutor 的 afterExecute（） 方法来处理
